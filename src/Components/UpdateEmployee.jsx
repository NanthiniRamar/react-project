...
const UpdateEmployee = ({setMode, onCompletion, selectedRow}) => {
    const[formValues, setFormValues] = useState(selectedRow) //store user input data
    const[errors, setErrors] = useState({}) // store error if exists
    ...
    const submitForm = () => {
        ...
        //find if the form object has values
        var objSize = Object.keys(formValues)?.length
        if(objSize > 0) var modifiedValues = formValues
        //call the api if the user only edit the their data
        if(modifiedValues != undefined || modifiedValues != null){
            if (isValid) {
            ...
            setMode('close')
            toast.dismiss(); 
            toast.success(`Nothing has been updated`, {
            position: toast.POSITION.TOP_RIGHT,
            });
        }
    }
    ...
    return(
        <div className="updateEmployee createEmployee">
            <Modal show>
                <Modal.Header>
                    <Modal.Title>
                        update Employee 
                ...
                <Modal.Body>
                    <div className="form-group employee-id">
                        ...
                        <input type="text" id="id" name="id" value={formValues.id} onChange={onChangeValues} required readOnly/>
